...
.. code-block:: python
    ...
    m = Map([
        # Static URLs
        Rule('/', endpoint='templates/index'),
        Rule('/about', endpoint='templates/about'),
        Rule('/help', endpoint='templates/help'),
        # Knowledge Base
        Subdomain('kb', [
...
.. code-block:: python
    ...
    c.build("templates/about")
    '/about'

    c.build("templates/index", force_external=True)
...
_rule_re = re.compile(
    r"""
    (?P<templates>[^<]*)                           # templates rule data
    <
    (?:
...
def parse_rule(rule: str) -> t.Iterator[t.Tuple[t.Optional[str], t.Optional[str], str]]:
    ...
    `None` it's a templates url part, otherwise it's a dynamic one.
    ...
    while pos < end:
        ...
        data = m.groupdict()
        if data["templates"]:
            yield None, None, data["templates"]
...
class Rule(RuleFactory):
    ...
    `build_only`
        ...
        or folder that are not handled by the WSGI application (like templates data)
    ...
    def match_compare_key(
        ...
        2.  rules with more templates parts come first so the second argument
            is the negative length of the number of the templates weights.
        3.  we order by templates weights, which is a combination of index
            and length
